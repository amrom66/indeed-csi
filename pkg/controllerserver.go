...
func (ind *indeed) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (resp *csi.CreateVolumeResponse, finalErr error) {
	if err := ind.validateControllerServiceRequest(csi.ControllerServiceCapability_RPC_CREATE_DELETE_VOLUME); err != nil {
	...
	if ind.config.EnableTopology {
		topologies = append(topologies, &csi.Topology{Segments: map[string]string{TopologyKeyNode: ind.config.NodeID}})
	}

	// Need to check for already existing volume name, and if found
	// check for the requested capacity and already allocated capacity
	if exVol, err := ind.state.GetVolumeByName(req.GetName()); err == nil {
	...
	vol, err := ind.createVolume(volumeID, req.GetName(), capacity, requestedAccessType, false /* ephemeral */, kind)
	if err != nil {
		return nil, err
	}
	...
	if req.GetVolumeContentSource() != nil {
		...
		case *csi.VolumeContentSource_Snapshot:
			if snapshot := volumeSource.GetSnapshot(); snapshot != nil {
				err = ind.loadFromSnapshot(capacity, snapshot.GetSnapshotId(), path, requestedAccessType)
				vol.ParentSnapID = snapshot.GetSnapshotId()
			}
		case *csi.VolumeContentSource_Volume:
			if srcVolume := volumeSource.GetVolume(); srcVolume != nil {
				err = ind.loadFromVolume(capacity, srcVolume.GetVolumeId(), path, requestedAccessType)
		...
		if err != nil {
			...
			if delErr := ind.deleteVolume(volumeID); delErr != nil {
...
func (ind *indeed) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	// Check arguments
	...
	if vol.Attached || !vol.Published.Empty() || !vol.Staged.Empty() {
		...
		if ind.config.CheckVolumeLifecycle {
			return nil, status.Error(codes.Internal, msg)
		}
		klog.Warning(msg)
	}

	if err := ind.deleteVolume(volId); err != nil {
...
func (ind *indeed) ControllerGetCapabilities(ctx context.Context, req *csi.ControllerGetCapabilitiesRequest) (*csi.ControllerGetCapabilitiesResponse, error) {
	return &csi.ControllerGetCapabilitiesResponse{
		Capabilities: ind.getControllerServiceCapabilities(),
	}, nil
}

func (ind *indeed) ValidateVolumeCapabilities(ctx context.Context, req *csi.ValidateVolumeCapabilitiesRequest) (*csi.ValidateVolumeCapabilitiesResponse, error) {
	...
	if _, err := ind.state.GetVolumeByID(req.GetVolumeId()); err != nil {
		return nil, err
	}
...
func (ind *indeed) ControllerPublishVolume(ctx context.Context, req *csi.ControllerPublishVolumeRequest) (*csi.ControllerPublishVolumeResponse, error) {
	if !ind.config.EnableAttach {
	...
	if req.NodeId != ind.config.NodeID {
		return nil, status.Errorf(codes.NotFound, "Not matching Node ID %s to hostpath Node ID %s", req.NodeId, ind.config.NodeID)
	}
	...
	if ind.config.AttachLimit > 0 && ind.getAttachCount() >= ind.config.AttachLimit {
		return nil, status.Errorf(codes.ResourceExhausted, "Cannot attach any more volumes to this node ('%s')", ind.config.NodeID)
	}
...
func (ind *indeed) ControllerUnpublishVolume(ctx context.Context, req *csi.ControllerUnpublishVolumeRequest) (*csi.ControllerUnpublishVolumeResponse, error) {
	if !ind.config.EnableAttach {
	...
	if req.NodeId != "" && req.NodeId != ind.config.NodeID {
		return nil, status.Errorf(codes.NotFound, "Node ID %s does not match to expected Node ID %s", req.NodeId, ind.config.NodeID)
	}
...
func (ind *indeed) GetCapacity(ctx context.Context, req *csi.GetCapacityRequest) (*csi.GetCapacityResponse, error) {
	// Lock before acting on global state. A production-quality
	// driver might use more fine-grained locking.
	ind.mutex.Lock()
	defer ind.mutex.Unlock()
	...
	available := ind.config.MaxVolumeSize
	if ind.config.Capacity.Enabled() {
		...
		// be set.
		kind := req.GetParameters()[storageKind]
		quantity := ind.config.Capacity[kind]
		allocated := ind.sumVolumeSizes(kind)
		available = quantity.Value() - allocated
	}
	maxVolumeSize := ind.config.MaxVolumeSize
...
func (ind *indeed) ListVolumes(ctx context.Context, req *csi.ListVolumesRequest) (*csi.ListVolumesResponse, error) {
	...
	for index := startIdx - 1; index < volumesLength && index < maxLength; index++ {
		...
		healthy, msg := ind.doHealthCheckInControllerSide(hpVolume.VolID)
...
func (ind *indeed) ControllerGetVolume(ctx context.Context, req *csi.ControllerGetVolumeRequest) (*csi.ControllerGetVolumeResponse, error) {
	// Lock before acting on global state. A production-quality
	// driver might use more fine-grained locking.
	ind.mutex.Lock()
	defer ind.mutex.Unlock()

	volume, err := ind.state.GetVolumeByID(req.GetVolumeId())
	if err != nil {
	...
	healthy, msg := ind.doHealthCheckInControllerSide(req.GetVolumeId())
...
func (ind *indeed) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) {
	if err := ind.validateControllerServiceRequest(csi.ControllerServiceCapability_RPC_CREATE_DELETE_SNAPSHOT); err != nil {
	...
	if exSnap, err := ind.state.GetSnapshotByName(req.GetName()); err == nil {
	...
	hostPathVolume, err := ind.state.GetVolumeByID(volumeID)
	if err != nil {
		return nil, err
	}
	...
	file := ind.getSnapshotPath(snapshotID)
	...
	if err := ind.state.UpdateSnapshot(snapshot); err != nil {
		return nil, err
	}
	return &csi.CreateSnapshotResponse{
		Snapshot: &csi.Snapshot{
...
func (ind *indeed) DeleteSnapshot(ctx context.Context, req *csi.DeleteSnapshotRequest) (*csi.DeleteSnapshotResponse, error) {
	// Check arguments
	...
	path := ind.getSnapshotPath(snapshotID)
	os.RemoveAll(path)
	if err := ind.state.DeleteSnapshot(snapshotID); err != nil {
		return nil, err
	}
	return &csi.DeleteSnapshotResponse{}, nil
}

func (ind *indeed) ListSnapshots(ctx context.Context, req *csi.ListSnapshotsRequest) (*csi.ListSnapshotsResponse, error) {
	if err := ind.validateControllerServiceRequest(csi.ControllerServiceCapability_RPC_LIST_SNAPSHOTS); err != nil {
	...
	if len(req.GetSnapshotId()) != 0 {
		...
		if snapshot, err := ind.state.GetSnapshotByID(snapshotID); err == nil {
	...
	// none if not found.
	if len(req.GetSourceVolumeId()) != 0 {
		for _, snapshot := range ind.state.GetSnapshots() {
	...
	hpSnapshots := ind.state.GetSnapshots()
...
func (ind *indeed) ControllerExpandVolume(ctx context.Context, req *csi.ControllerExpandVolumeRequest) (*csi.ControllerExpandVolumeResponse, error) {
	...
	if capacity > ind.config.MaxVolumeSize {
		return nil, status.Errorf(codes.OutOfRange, "Requested capacity %d exceeds maximum allowed %d", capacity, ind.config.MaxVolumeSize)
	}
	...
	exVol, err := ind.state.GetVolumeByID(volID)
	if err != nil {
		return nil, err
	}
	...
	if exVol.VolSize < capacity {
		...
		if err := ind.state.UpdateVolume(exVol); err != nil {
			return nil, err
		}
	}
...
func (ind *indeed) validateControllerServiceRequest(c csi.ControllerServiceCapability_RPC_Type) error {
	if c == csi.ControllerServiceCapability_RPC_UNKNOWN {
		return nil
	}

	for _, cap := range ind.getControllerServiceCapabilities() {
...
func (ind *indeed) getControllerServiceCapabilities() []*csi.ControllerServiceCapability {
	var cl []csi.ControllerServiceCapability_RPC_Type
	if !ind.config.Ephemeral {
		...
		if ind.config.EnableVolumeExpansion && !ind.config.DisableControllerExpansion {
